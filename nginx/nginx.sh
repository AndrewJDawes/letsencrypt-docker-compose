...
#     sed -i "s|/etc/letsencrypt/live/$1|/etc/nginx/site_certs/$1|g" "/etc/nginx/sites/$1.conf"
#   fi
# }

# use_lets_encrypt_certificate() {
#   if grep -q "/etc/nginx/site_certs/$1" "/etc/nginx/sites/$1.conf"; then
#     echo "Switching Nginx to use Let's Encrypt certificate for $1"
#     sed -i "s|/etc/nginx/site_certs/$1|/etc/letsencrypt/live/$1|g" "/etc/nginx/sites/$1.conf"
#   fi
# }
